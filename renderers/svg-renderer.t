...
/*
        local test = ConnectionMapShape.builder()
                        .start(0,0)
                        .end(60,60)
                        .attributes('kalle')
                        .style('styleeee')
...
class ConnectionMapShape: object 
    x1 = nil
    y1 = nil
    x2 = nil
    y2 = nil
    ...
    endMarker = nil
    color = nil
    //dashedAttribute = 'stroke-dasharray="10 5"'
    renderStr = '<line x1 = "<<x1>>" y1 = "<<y1>>" x2 = "<<x2>>" y2 = "<<y2>>" <<attributes>> style = "<<style>>" />'
    arrowId = 'arrow'

    construct(x1?,y1?,x2?,y2?, style?, attributes?) {
        self.x1 = x1;
        self.y1 = y1;
        self.x2 = x2;
        self.y2 = y2;
        self.style = style? style : gMapThemes.getLineStyle();
        self.attributes = attributes? attributes : gMapThemes.getLineAttributes();
    }


    useMarkers(startMarker, endMarker) {
    ...
    render() {
        if(startMarker) {
            attributes += ' marker-start="url(#<<arrowId>>)" ';
        }        
        if(endMarker) {
            attributes += ' marker-end="url(#<<arrowId>>)" ';
        }
        return renderStr;
    }
    ...
    setDashed(values) {
        self.attributes += ' stroke-dasharray="<<values>>" ';
    }

    // TODO: StringBuffer
    renderCrooked(direction, flipped?, xValue?, yValue?) {
        xValue = xValue? xValue: 30;
        yValue = yValue? yValue: 30;
        if(direction==&northeast) {
            renderStr = '<path d="M <<x1>>,<<y1>>  <<x1>>,<<y1-yValue>>  <<x2-xValue>>,<<y2>> <<x2>>,<<y2>>"';
        }
        if(direction==&southwest) {
            if(flipped) {
                renderStr = '<path d="M <<x1>>,<<y1>>  <<x1>>,<<y1+yValue>>  <<x2+xValue>>,<<y2>> <<x2>>,<<y2>>"';
            } else {
                renderStr = '<path d="M <<x1>>,<<y1>>  <<x1-xValue>>,<<y1>>  <<x2>>,<<y2-yValue>> <<x2>>,<<y2>>"';
            }
        }
        if(direction==&southeast) {
            if(flipped) {
        ...
        if(self.startMarker == true) {
            attributes += ' marker-start="url(#<<arrowId>>)" ';
        }
        
        if(self.endMarker == true) {
            attributes += ' marker-end="url(#<<arrowId>>)" ';
        }
        renderStr += ' <<attributes>>  style="<<style>>" />';
        return renderStr;
    }

    /*dontusebuilder: ConnectionMapShape {
        x1=0
        y1=0
        x2=0
        y2=0
        ...
        build(x1?, y1?, x2?, y2?, attributes?, style?) {
            self.x1 = x1? x1: self.x1;
            self.y1 = y1? y1: self.y1;
            self.x2 = x2? x2: self.x2;
            self.y2 = y2? y2: self.y2;
            self.attr = attributes? attributes: self.attr;
            self.stl = style? style: self.stl;            
            local shape = new ConnectionMapShape(self.x1,self.y1,self.x2,self.y2);
            if(attr) {
                shape.attributes = attr;
            }   
            if(stl) {
                shape.style = stl;
            } 
            if(self.str) {
                shape.renderStr = self.str;
            }
            shape.useMarker(startMarker, endMarker);
            clear();
            return shape;
        }
        clear() {
        ...
        dashed(values) {
            if(self.attr) {
                self.attr += ' stroke-dasharray="<<values>>" ';
            } else {
                self.attr = ' stroke-dasharray="<<values>>" ';
            }
            return self;
        ...
        buildCrooked(direction, flipped?, xValue?, yValue?) {
            local shape = build();
            xValue = xValue? xValue: 30;
            yValue = yValue? yValue: 30;
            if(direction==&northeast) {
                shape.renderStr = '<path d="M <<shape.x1>>,<<shape.y1>>  <<shape.x1>>,<<shape.y1-yValue>>  <<shape.x2-xValue>>,<<shape.y2>> <<shape.x2>>,<<shape.y2>>"';
            }
            if(direction==&southwest) {
                if(flipped) {
                    shape.renderStr = '<path d="M <<shape.x1>>,<<shape.y1>>  <<shape.x1>>,<<shape.y1+yValue>>  <<shape.x2+xValue>>,<<shape.y2>> <<shape.x2>>,<<shape.y2>>"';
                } else {
                    shape.renderStr = '<path d="M <<shape.x1>>,<<shape.y1>>  <<shape.x1-xValue>>,<<shape.y1>>  <<shape.x2>>,<<shape.y2-yValue>> <<shape.x2>>,<<shape.y2>>"';
                }
            }
            if(direction==&southeast) {
                if(flipped) {
                    shape.renderStr = '<path d="M <<shape.x1>>,<<shape.y1>>  <<x1>>,<<y1+yValue>>  <<shape.x2-xValue>>,<<shape.y2>> <<shape.x2>>,<<shape.y2>>"';
                } else {
                    shape.renderStr = '<path d="M <<shape.x1>>,<<shape.y1>>  <<shape.x1+xValue>>,<<shape.y1>>  <<shape.x2>>,<<shape.y2-yValue>> <<shape.x2>>,<<shape.y2>>"';
                }
            }
            
            shape.renderStr += ' <<shape.attributes>> style="<<shape.style>>" ';
            clear();
            return shape;
        }
    ...
    function createGrayLine(x1,y1,x2,y2) {
        return '<line x1 = "<<x1>>" y1 = "<<y1>>" x2 = "<<x2>>" y2 = "<<y2>>" style = "stroke:gray;stroke-width:2" />';
    }
    function createRedLine(x1,y1,x2,y2) {
        return '<line x1 = "<<x1>>" y1 = "<<y1>>" x2 = "<<x2>>" y2 = "<<y2>>" style = "stroke:red;stroke-width:2" />';
    }

    function createBlueLine(x1,y1,x2,y2) {
        return '<line x1 = "<<x1>>" y1 = "<<y1>>" x2 = "<<x2>>" y2 = "<<y2>>" style = "stroke:blue;stroke-width:2" />';
    }
    ...
    function createCrookedLine(x1,y1,x2,y2, direction, flipped?, xValue?, yValue?, attr?) {
        attr = attr? attr : gMapThemes.getLineAttributes();
        ...
        if(direction==&southwest) {
            if(flipped) {
                return '<path d="M <<x1>>,<<y1>>  <<x1>>,<<y1+yValue>>  <<x2+xValue>>,<<y2>> <<x2>>,<<y2>>"
                    <<attr>> style = "<<gMapThemes.getLineStyle()>>" />';
            }
            return '<path d="M <<x1>>,<<y1>>  <<x1-xValue>>,<<y1>>  <<x2>>,<<y2-yValue>> <<x2>>,<<y2>>"
                <<attr>> style = "<<gMapThemes.getLineStyle()>>" />';
        }
